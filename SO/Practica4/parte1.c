...
dev_t start; // representa el primer par de mayores y menores que se tratan de reservar en el drive
...
int init_module(void)
{
    ...
    if ((chardev=cdev_alloc())==NULL) {
        ...
        return -ENOMEM;
    } // crear estructura cdev y le da valor chardev


    cdev_init(chardev,&fops); // asignación operaciones
    ...
    if ((ret=cdev_add(chardev,start,1))) {
        ...
        unregister_chrdev_region(start, 1);
        return ret;
    } // asociar un rango de mayor menor a la estrucutra dev mediante paramtero start

    major=MAJOR(start); // obtener valor mayor y minor
    minor=MINOR(start);
...
void cleanup_module(void)
{
    /* Destroy chardev */
    if (chardev)
        cdev_del(chardev); // Destruye la estructura

    /*
     * Unregister the device
     */
    unregister_chrdev_region(start, 1); // Eimina el dispositivo
}
...
device_write(struct file *filp, const char *buff, size_t len, loff_t * off)
{
    ...
    int leds = 0x0; int i = 0;
    char aux[BUF_LEN];
    bool iz = false, d = false, c = false;
    // iz num lock / c caps lock / d scroll lock
